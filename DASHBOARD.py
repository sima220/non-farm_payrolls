...
def load_data():
    ...
    try:
        conn = psycopg2.connect(
            dbname=os.environ.get("DB_NAME", "NFP_ETL"),
            user=os.environ.get("DB_USER", "postgres"),
            password=os.environ.get("DB_PASSWORD", "seemaxime@30190"),
...
def create_slicing_charts(df):
    ...
    # Bar chart: Average payroll employment by calendar year (2010-2025)
    st.subheader("Average Total Payroll Employment by Year (Bar Chart)")
    df_yearly = df[(df['date'].dt.year >= 2010) & (df['date'].dt.year <= 2025)].copy()
    df_yearly['year'] = df_yearly['date'].dt.year
    avg_by_year = df_yearly.groupby('year')['total_nonfarm'].mean().reset_index()
    avg_by_year.rename(columns={'total_nonfarm': 'Average Employment'}, inplace=True)
    fig_bar = px.bar(
        avg_by_year,
        x='year',
        y='Average Employment',
        title="Average Total Payroll Employment by Year (2010-2025)",
        labels={'year': 'Year', 'Average Employment': 'Average Employment (in thousands)'},
        text_auto=True
    )
...
def create_roll_up_charts(df):
    ...
    st.plotly_chart(fig_decades, key="decades_bar_chart")

    # Interactive decade comparison
    st.subheader("Compare Average Employment Across Decades")
    min_year = int(df['date'].dt.year.min())
    max_year = int(df['date'].dt.year.max())
    start_decade = st.slider(
        "Select first decade start year:",
        min_value=min_year,
        max_value=max_year - 9,
        value=2000,
        step=10
    )
    end_decade = st.slider(
        "Select second decade start year:",
        min_value=min_year,
        max_value=max_year - 9,
        value=2010,
        step=10
    ...
    df['year'] = df['date'].dt.year
    decade_1 = df[(df['year'] >= start_decade) & (df['year'] <= start_decade + 9)]
    decade_2 = df[(df['year'] >= end_decade) & (df['year'] <= end_decade + 9)]

    avg_decade_1 = decade_1['total_nonfarm'].mean()
    avg_decade_2 = decade_2['total_nonfarm'].mean()

    compare_decades_df = pd.DataFrame({
        'Decade': [f"{start_decade}s", f"{end_decade}s"],
        'Average Employment': [avg_decade_1, avg_decade_2]
    })

    fig_compare_decades = px.bar(
        compare_decades_df,
        x='Decade',
        y='Average Employment',
        title=f"Average Employment: {start_decade}s vs. {end_decade}s",
        labels={'Average Employment': 'Average Employment (in thousands)'}
    )
    st.plotly_chart(fig_compare_decades, use_container_width=True, key="interactive_decades_bar_chart")
...
def main():
    ...
    # --- Population and President Timeline ---
    st.subheader("U.S. Population and Presidential Timeline (2010-2025)")
    ...
    # Example president timeline
    president_data = [
        {"President": "Barack Obama", "Start": 2009, "End": 2016},
        {"President": "Donald Trump", "Start": 2017, "End": 2020},
        {"President": "Joe Biden", "Start": 2021, "End": 2025}
    ]

    # Prepare DataFrame for display
    pop_pres_df = pd.DataFrame({
        "Year": list(population_data.keys()),
        "Population (millions)": list(population_data.values()),
        "President": [
            next(p["President"] for p in president_data if p["Start"] <= year <= p["End"])
            for year in population_data.keys()
        ]
    })

    st.dataframe(pop_pres_df)

    # Optional: Timeline chart for presidents
    ...
    for p in president_data:
        fig_pres.add_trace(go.Bar(
            ...
            hovertext=f"{p['President']} ({p['Start']}-{p['End']})"
        ))
    fig_pres.update_layout(
        title="U.S. Presidential Timeline (2010-2025)",
        xaxis_title="Year",
        yaxis_visible=False,
        barmode='stack',
        showlegend=True
    )
    st.plotly_chart(fig_pres, use_container_width=True, key="president_timeline")
